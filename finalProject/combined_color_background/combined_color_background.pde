...
// CODE SOURCES: https://processing.org/examples/lineargradient.html
...
void draw(){
  ...
  if(instructions == true & universe == false){
    ...
    textFont(font, 20);
    text("G O   T O   S P A C E", 300, 450);
    if (transition == true){
      textColor -= 5;
      if (textColor < 0){
        instructions = false;
...
void mousePressed(){
  if (intro == true){
    if ((mouseX < 350) && (mouseX > 250) && ((mouseY < 430) && (mouseY > 370))) {
      transition = true;
    }
  }
  if (instructions == true){
    if ((mouseX < 360) && (mouseX > 240) && ((mouseY < 470) && (mouseY > 430))) {
      transition = true;
    }
  }
...
void keyPressed(){
  ...
  if (intro == false & instructions == false){
    if (key == 'h'){
      help = !help;
      universe = !universe;
    }
